...
The chat uses an **iMessage-style visual theme** ("iMessage Air"): borderless gray received bubbles, blue-gradient sent bubbles (`#0a84ff → #0066f5`), translucent blurred chrome, pill input, no in-thread avatars. It is implemented as `--im-*` CSS variables + `.im-*` utility classes in each app's `globals.css`, with light and dark values keyed to each app's theme mechanism (CRM: dark default / `:root.light`; portal: light default / `:root.dark`). The theme is palette-independent — CRM accent palettes don't affect the chat. Functionally it carries structured messages beyond plain text. `Message.type` is one of `text | file | document_request | form_request | form_response | system`, with a `meta` payload per type and an optional `replyTo` snapshot (`{ messageId, senderName, preview }`) for quoted replies.
